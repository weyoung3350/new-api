...
try:
    ...
    def create_layout_excel():
        ...
        # 标题
        ws_layout["A1"] = "露台石英砖铺设布局图 (73.6平方米)"
        ...
        for i, (key, value) in enumerate(info_data):
            ...
            if key == "项目信息" or key == "图例说明":
                ...
                ws_layout[f"B{row}"].font = Font(bold=True, size=11)
            if key == "●":
                ws_layout[f"A{row}"].fill = support_fill
                ws_layout[f"A{row}"].font = Font(bold=True, color="FFFFFF")
            elif key == "砖X":
        ...
        # 网格说明
        ws_layout[f"A{grid_start_row-1}"] = "铺设布局网格图 (每格代表300mm)"
        ws_layout[f"A{grid_start_row-1}"].font = Font(bold=True, size=11)
        ...
        for col_idx in range(21):  # A到U共21列
            ...
            ws_layout[f"{col_letter}{grid_start_row}"].border = thin_border
        ...
        for row_idx in range(22):  # 22行网格
            ...
            ws_layout[f"A{excel_row}"].border = thin_border
            ...
            for col_idx in range(1, 21):  # B到U列
                ...
                # 石英砖区域
                elif row_idx % 2 == 1 and col_idx % 2 == 0:
        ...
        for col_idx in range(1, 22):
            ...
            ws_layout.column_dimensions[col_letter].width = 5
        ...
        for row_idx, row_data in enumerate(calc_data, 1):
            for col_idx, value in enumerate(row_data, 1):
                ...
                # 设置样式
                if row_idx in [2, 8, 13, 20, 28]:  # 标题行
